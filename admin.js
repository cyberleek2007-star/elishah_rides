...
function bind(){
 ...
 $('#bookingSearch').addEventListener('input',renderBookings);$('#bookingStatus').addEventListener('change',renderBookings);$('#clearBookingFilters').addEventListener('click',()=>{$('#bookingSearch').value='';$('#bookingStatus').value='';renderBookings()});$('#refreshPendingBookings').addEventListener('click',async()=>{await loadBookings();renderAll()});
...
function renderAll(){renderStats();renderLatest();renderPendingBookings();renderBookings();renderFleet();renderBlocks();renderCustomers();renderContent('tours');renderContent('destinations');renderContent('pricing');renderContent('testimonials');renderGallery();renderMessages();renderCommunity();fillBlockVehicles()}
...
function renderPendingBookings(){const el=$('#pendingBookingsList'),countEl=$('#pendingBookingsCount');if(!el)return;const pending=bookings.filter(b=>String(b.status||'Pending').toLowerCase()==='pending');if(countEl)countEl.textContent=pending.length;if(!pending.length){el.innerHTML='<div class="pending-empty"><span>✓</span><div><strong>No pending bookings</strong><small>New booking requests will appear here automatically.</small></div></div>';return}el.innerHTML=pending.map(b=>`<article class="pending-booking"><div class="pending-main"><div class="pending-ref"><strong>${esc(b.ref||'Booking')}</strong><span class="status pending">Pending</span></div><strong class="pending-name">${esc(b.name||'Traveller')}</strong><span class="pending-route">${esc(b.pickup||'')} → ${esc(b.destination||'')}</span><small>${esc(b.travel_date||'—')} · ${esc(b.pickup_time||'—')} · ${esc(b.service||'Chauffeur Hire')}</small></div><div class="pending-actions"><button class="small-btn" data-action="view-booking" data-id="${esc(b.id)}">View</button><button class="confirm-btn" data-action="confirm-booking" data-id="${esc(b.id)}">✓ Confirm</button><button class="delete-btn" data-action="delete-booking" data-id="${esc(b.id)}">Delete</button></div></article>`).join('')}
...
async function updateBooking(id,nextStatus=null){const b=bookings.find(x=>x.id===id);if(!b)return;const current=b.status||'Pending';const opts=['Pending','Confirmed','Paid','Cancelled','Completed'];const next=nextStatus||prompt('Set booking status:\n\n'+opts.join(' / '),current);if(!next||!opts.includes(next))return;const patch={status:next};if(next==='Paid')patch.payment_status='Paid';const {error}=await sb.from('bookings').update(patch).eq('id',id);if(error){toast(error.message);return}toast(next==='Confirmed'?'Booking confirmed':'Booking status updated');await loadBookings();renderAll()}
async function deleteBooking(id){const b=bookings.find(x=>x.id===id);if(!b)return;const ref=b.ref||'this booking';if(!confirm(`Delete ${ref}?\n\nThis will permanently remove the booking from the admin list.`))return;const {error}=await sb.from('bookings').delete().eq('id',id);if(error){toast(error.message);return}toast('Booking deleted');await loadBookings();renderAll()}
...
function handleActions(e){const b=e.target.closest('[data-action]');if(b){const id=b.dataset.id;switch(b.dataset.action){case'view-booking':bookingModal(id);break;case'status':updateBooking(id);break;case'confirm-booking':updateBooking(id,'Confirmed');break;case'delete-booking':deleteBooking(id);break;case'edit-vehicle':vehicleModal(id);break;case'toggle-vehicle':toggleVehicle(id);break;case'delete-block':deleteBlock(id);break;case'edit-content':contentModal(b.dataset.type,id);break;case'delete-content':deleteContent(b.dataset.type,id);break;
