...
function renderPendingBookings(){const el=$('#pendingBookingsList'),countEl=$('#pendingBookingsCount');if(!el)return;const pending=bookings.filter(b=>bookingStatus(b).toLowerCase()==='pending');if(countEl)countEl.textContent=pending.length;if(!pending.length){el.innerHTML='<div class="pending-empty"><span>✓</span><div><strong>No pending bookings</strong><small>New booking requests will appear here automatically.</small></div></div>';return}el.innerHTML=pending.map(b=>`<article class="pending-booking"><div class="pending-main"><div class="pending-ref"><strong>${esc(b.ref||'—')}</strong><span class="status pending">Pending</span></div><h4>${esc(b.name||'Traveller')}</h4><p>${esc(b.service||'Journey')} · ${esc(b.pickup||'')} → ${esc(b.destination||'')}</p><small>${esc(b.travel_date||'—')} ${esc(b.pickup_time||'')} · ${esc(b.vehicle_unit_name||b.vehicle||'—')}</small></div><div class="pending-actions"><button class="small-btn" data-action="view-booking" data-id="${esc(b.id)}">View</button><button class="small-btn confirm-btn" data-action="confirm-booking" data-id="${esc(b.id)}">✓ Confirm</button><button class="small-btn danger-btn" data-action="delete-booking" data-id="${esc(b.id)}">Delete</button></div></article>`).join('')}
function bookingRowActions(b){const s=bookingStatus(b).toLowerCase();let actions=`<button class="small-btn" data-action="view-booking" data-id="${esc(b.id)}">View</button>`;if(s==='pending')actions+=`<button class="small-btn confirm-btn" data-action="confirm-booking" data-id="${esc(b.id)}">✓ Confirm</button><button class="small-btn danger-btn" data-action="delete-booking" data-id="${esc(b.id)}">Delete</button>`;else if(s==='confirmed'||s==='paid')actions+=`<button class="small-btn danger-btn" data-action="cancel-booking" data-id="${esc(b.id)}">✕ Cancel</button><button class="small-btn danger-btn" data-action="delete-booking" data-id="${esc(b.id)}">Delete</button>`;else if(s==='cancelled')actions+=`<button class="small-btn danger-btn" data-action="delete-booking" data-id="${esc(b.id)}">Delete</button>`;return actions}
function renderBookings(){const q=($('#bookingSearch')?.value||'').toLowerCase();const st=($('#bookingStatus')?.value||'').toLowerCase();const rows=bookings.filter(b=>{const hay=[b.ref,b.name,b.phone,b.pickup,b.destination,b.vehicle,b.service].join(' ').toLowerCase();return (!q||hay.includes(q))&&(!st||bookingStatus(b).toLowerCase()===st)});$('#bookingsTable').innerHTML=rows.length?rows.map(b=>`<tr><td><strong>${esc(b.ref||'—')}</strong><small>${esc(b.created_at?new Date(b.created_at).toLocaleString(): '')}</small></td><td><strong>${esc(b.name||'—')}</strong><small>${esc(b.phone||'')}</small></td><td class="route-cell"><strong>${esc(b.service||'—')}</strong><span>${esc(b.pickup||'')} → ${esc(b.destination||'')}</span></td><td>${esc(b.travel_date||'—')}<br><small>${esc(b.pickup_time||'')}</small></td><td>${esc(b.vehicle_unit_name||b.vehicle||'—')}</td><td>${esc(b.payment_method||'—')}<br><small>${esc(b.payment_status||'Unpaid')}</small></td><td><span class="status ${bookingStatus(b).toLowerCase()}">${esc(bookingStatus(b))}</span></td><td><div class="row-actions">${bookingRowActions(b)}</div></td></tr>`).join(''):'<tr><td colspan="8"><div class="empty-state">No bookings match your filters.</div></td></tr>'}
...
async function deleteBooking(id){const b=bookings.find(x=>x.id===id);if(!b)return;if(!confirm(`Permanently delete booking ${b.ref||''}? This cannot be undone.`))return;const {error}=await sb.from('bookings').delete().eq('id',id);if(error){toast(error.message);return}await loadBookings();renderAll();toast('Booking deleted')}
async function updateBooking(id){const b=bookings.find(x=>x.id===id);if(!b)return;const current=bookingStatus(b);const opts=['Pending','Confirmed','Paid','Cancelled','Completed'];const next=prompt('Set booking status:\n\n'+opts.join(' / '),current);if(!next||!opts.includes(next))return;if(await setBookingStatus(id,next))toast('Booking status updated')}
