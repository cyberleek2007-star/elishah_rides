...
async function submitCommunityPhoto(e){
 ...
 const ins=await sb.from('community_photos').insert({name:f.name.value.trim(),caption:f.caption.value.trim()||null,storage_path:path,status:'pending'});if(ins.error){await sb.storage.from('community-gallery').remove([path]);msg.textContent=ins.error.message;return;}
 msg.textContent='Submitted for review. Thank you!';f.reset();setTimeout(()=>closeCommunityModal('communityUploadModal'),1100);
}
async function submitCommunityComment(e){e.preventDefault();const f=e.target,msg=document.getElementById('communityCommentMsg');if(!sb){msg.textContent='Service is not connected yet.';return;}msg.textContent='Posting…';const {error}=await sb.from('community_comments').insert({name:f.name.value.trim(),comment:f.comment.value.trim(),status:'pending'});if(error){msg.textContent=error.message;return;}msg.textContent='Submitted for review. Thank you!';f.reset();setTimeout(()=>{closeCommunityModal('communityCommentModal');loadCommunity()},1100);}
